  ...
  ({ children, className, label, error, ...props }, ref) => (
    <label className="flex w-full flex-col gap-2 text-sm font-medium text-slate-700">
      ...
      <select
        ref={ref}
        className={cn(
          "min-h-12 rounded-[10px] border border-slate-200 bg-white px-4 text-base text-slate-900 outline-none transition focus:border-[var(--color-primary)] focus:ring-4 focus:ring-[color-mix(in_srgb,var(--color-primary)_14%,white)]",
